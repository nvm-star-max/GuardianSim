#!/usr/bin/env python3
"""Add natural Qwen narration and fixed captions to the 80-second review cut."""
...
CHAPTERS = [
    ...
    Chapter(
        ...
        "Formal Seed 411",
        (
            "Here is seed four eleven from the frozen test set. The original path "
            "overlaps the clutter by one point four two millimeters. The selected "
            "alternative leaves seventeen point one millimeters of clearance and "
            "repeats safely three times."
        ),
    ...
    Chapter(
        ...
        (
            "We then ran the frozen thirty-scenario set. Repeatable safe completion "
            "went from eighteen cases to all thirty. Across the ninety independent "
            "executions, sampled clutter contacts went from thirty to zero."
        ),
...
def build_audio() -> list[dict]:
    ...
    total_samples = round(80.0 * sample_rate)
