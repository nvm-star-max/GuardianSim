...
def validate(sidecar: Path = DEFAULT_SIDECAR) -> dict:
    ...
    require(metrics["largest_batch_throughput_p50"] == 278051, "P50 throughput claim drift")
    require(metrics["largest_batch_throughput_p95"] == 278660, "P95 throughput claim drift")
    require(metrics["largest_batch_throughput_min"] == 274990, "Minimum throughput claim drift")
    require(metrics["largest_batch_throughput_max"] == 278672, "Maximum throughput claim drift")
    require(metrics["largest_vs_smallest_batch_p50_ratio"] == 1.82, "Scale ratio drift")
    ...
    require(metrics["peak_vram_used_bytes"] == 23677100032.0, "Peak VRAM claim drift")
